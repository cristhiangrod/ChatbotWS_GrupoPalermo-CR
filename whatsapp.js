...
class Whatsapp {
    constructor(idCliente, celular, alias, session, callback) {
        ...
        if (session === null) {
            ...
            if (verificacion === undefined) {
                console.log('Nuevo...');
                this.client = new Client({authStrategy: new NoAuth() });
    ...
    enviarMensaje = (celular, mensaje, archivo, callback) => {
        ...
        if (archivo !== '' && archivo != undefined) {
            ...
            this.client.sendMessage(`${celular}@c.us`, media).then(function () { }).catch(function () { });
        }
    }
    onload(idCliente, celular, alias, callback) {
        ...
        this.client.on('authenticated', (session) => {
            ...
            this.session = { authStrategy : 'NoAuth'};
            this.autenticado = true;
            data.consultar(STORE_REGISTRAR, [idCliente, celular, alias, JSON.stringify(this.session), alias, JSON.stringify(this.session)]);
        });
